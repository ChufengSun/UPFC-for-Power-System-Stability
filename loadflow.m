...
%bus_int - 	internal bus number index
%Qg - 		generator reactive power vector 
%bus_type  - 	1 for swing bus; 2 for generator bus; 3 for load bus
%g_bno  -		 vector to identify generator buses
%PQV_no  - 	generator bus index
%PQ_no     -	 load bus index
%ang_red   -	 matrix to eliminate swing bus angles
%volt_red  -	 matrix to eliminate swing and generator bus voltages
%Q - 		reactive power at bus
%Ql - 		load reactive power at bus 
%
%Inputs:
%bus - 		ac bus specification matrix
%line - 		ac line specification matrix
%tol - 		the load flow tolerance
%iter_max - 	the maximum number of load flow iterations
%acc - 		the acceleration (acc >1) or  deceleration (acc <1) factor for improving the convergence of the Newton-Raphson iteration. 
%display  -  	'y' creates a load flow report,  'n' suppresses a load flow report
%flag  - 		1, a new Jacobian is calculated at each  iteration
%Outputs:
%bus_sol - 	contains the solved power flow bus data
%line_sol - 	contains the solved power flow line data
%line_flw - 	contains the solved line power flows
...
%12	G gen min	pu on system base
...
%10	tap step	
%
%It is assumed that the tap and the phase shifter are located at the from bus. There is no need to input tap and phase shifter data if there are none.
%
%% See also:  
%%
%% Algorithm: Newton-Raphson method using the polar form of 
%%   the equations for P(real power) and Q(reactive power).
%%largest bus power mismatch is less than the tolerance tol or the number of  iterations exceeds iter_max. 
