...
% part of s_simu
% calls red_ybus
% corrected unballanced fault - reported by Ian Hiskens - February 1999

% create pre-fault admittance matrix
if ~isempty(load_con);nload = length(load_con(:,1));end
[Y_gprf,Y_gncprf,Y_ncgprf,Y_ncprf,V_rgprf,V_rncprf,boprf,Y_upfcprf,V_upfcprf] = red_ybus(bus,line);
...
if f_type <4
   ...
   if f_type == 0
      ...
      bf = 1.0/1e-7;
   elseif f_type ==1
      % line to ground fault zf = zn + z0 
      xf = sw_con(2,3) + sw_con(2,4);
      bf = 1.0/xf;
   elseif f_type == 2
      % line to line to ground  zf = zn*z0/(zn+z0)
      xf = sw_con(2,3)*sw_con(2,4)/(sw_con(2,3) + sw_con(2,4));
      bf = 1.0/xf;
   elseif f_type==3
      %line to line  zf = zn
      xf = sw_con(2,3);
...
[Y_gf,Y_gncf,Y_ncgf,Y_ncf,V_rgf,V_rncf,bof,Y_upfcf,V_upfcf] = red_ybus(bus_f,line_f);   % fault-on
% admittance matrix
bus_intf = bus_int;
...
if f_type<4
   ...
   [Y_gpf1,Y_gncpf1,Y_ncgpf1,Y_ncpf1,V_rgpf1,V_rncpf1,bopf1,Y_upfcpf1,V_upfcpf1]...
      = red_ybus(bus_pf1,line_pf1);  % post-fault
   % admittance matrix
   bus_intpf1 = bus_int;
else
   % fault type is 4 or 5, or 6 no change in system structure
...
if f_type<4
   ...
   [Y_gpf2,Y_gncpf2,Y_ncgpf2,Y_ncpf2,V_rgpf2,V_rncpf2,bopf2,Y_upfcpf2,V_upfcpf2]...
      = red_ybus(bus_pf2,line_pf2);  % post-fault
   % admittance matrix
   bus_intpf2 = bus_int;
else
   % load type = 4 or 5 or 6
